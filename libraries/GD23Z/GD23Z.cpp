...
#define TOUCH_DATA_U8 \
 ...
 242,83,11,255,2,70,214,226,128,0,0,26,255,255,255,20,33,48,0,4,0,0,0,15,0,0,0,26,255,255,255,32,32,48,0,4,0,0,0,0,0,0,0
/* FT811 / FT813 binary-blob from FTDIs AN_336 to patch the touch-engine for Goodix GT911 / GT9271 touch controllers */
static const uint8_t eve_gt911_data[TOUCH_DATA_LEN] = {TOUCH_DATA_U8};

static bool coprocessor_busy(void)
{
  uint16_t space = GDTR.rd16(REG_CMDB_SPACE);
	Serial.printf("%s(): space=0x%x\n", __FUNCTION__, space);
	return space != 0xffcU;
}


static void wait_coprocessor_idle(void)
{
	while (coprocessor_busy()) delay(2U);
}


// Goodix GT911 capacitive touch support
void GDClass::gt911(void)
{
  GDTR.wr_n(REG_CMDB_WRITE, (uint8_t*)eve_gt911_data, sizeof(eve_gt911_data));
  wait_coprocessor_idle();
  GDTR.wr(REG_TOUCH_OVERSAMPLE, 0x0fU); /* setup oversample to 0x0f as "hidden" in binary-blob for AN_336 */
  GDTR.wr16(REG_TOUCH_CONFIG, 0x05D0U);  /* write magic cookie as requested by AN_336 */

  /* specific to the EVE2 modules from Matrix-Orbital we have to use GPIO3 to reset GT911 */
  GDTR.wr16(REG_GPIOX_DIR, 0x8008U); /* make GPIO3 to output, default-value */
  delay(2U);  /* wait more than 100us */
  GDTR.wr(REG_CPURESET, 0U);        /* clear all resets */
  delay(110U); /* wait more than 55ms - does not work with multitouch, for some reason a minimum delay of 108ms is required */
  GDTR.wr16(REG_GPIOX_DIR, 0x8000U); /* setting GPIO3 back to input */
}
...
void GDClass::begin(uint8_t options) {
...
#if 1
  Serial.printf("ID REGISTER: 0x%x\r\n", GDTR.rd(REG_ID));
#endif
  ...
  if ((v8[1] == 0xff) && (v8[2] == 0x01)) {
    Serial.printf("%s/%s().%d: ***NOT*** calling GD.display_setup()\r\n", __FILE__, __FUNCTION__, __LINE__); Serial.flush();
    ...
    Serial.printf("%s/%s().%d: GD.display_setup()\r\n", __FILE__, __FUNCTION__, __LINE__); // LCE-GRH: goes here...
    GD.display_setup();
  }
  ...
  cmd_regwrite(REG_PWM_DUTY, SCREEN_FT81X==14? 102:128);
...
void GDClass::display_setup(void) {
#if(SCREEN_FT81X == 13)
// ******************************************************************************************
...
void GDClass::self_calibrate(void) {
  ...
  #if SCREEN_FT81x == 14
  y_offset = 130;
  #elif SCREEN_FT81x == 15
  y_offset = 110;
  #elif SCREEN_FT81x == 16
  y_offset = 50;
  #endif

  if (y_offset) GD.Vertex2ii((GD.w / 2)-50, (GD.h / 2)-y_offset, 0, 0);
...
void GDClass::cmd_n(uint8_t* s, uint32_t n) {
  byte a = n & 3;
  while (n > 0) {
    uint16_t part = n > 512? 512 : n; 
    GDTR.cmd_n((byte*)s, part);
    s += part;
    n -= part;
  }
  align(a);
...
byte GDClass::load(const char *filename, void (*progress)(long, long))
{
  ...
  if (r.openfile(filename)) {
    ...
    while (r.offset < r.size) {
      uint16_t n = min(512UL, r.size - r.offset);
