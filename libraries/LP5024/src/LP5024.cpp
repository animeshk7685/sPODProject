 ...
 */
LP5024::LP5024(LED_Configuration ledConfiguration) {
 ...
 */
LP5024::LP5024(uint8_t enablePin) {
    if (enablePin != 0xFF) pinMode(enablePin, OUTPUT);
 ...
 * @param enablePin the pin that is connected to the EN pin of the LP5009 or LP5012
 */
LP5024::LP5024(LED_Configuration ledConfiguration, uint8_t enablePin) {
    _led_configuration = ledConfiguration;
 ...
 */
void LP5024::Begin(uint8_t i2cAddress) {
    i2c1_init();
    ...
    i2c1_write_byte(_i2c_address, DEVICE_CONFIG0, 1 << 6);
}
 ...
 */
void LP5024::ResetRegisters(EAddressType addressType) {
    i2c1_write_byte(getAddress(addressType), RESET_REGISTERS, 0xFF);
}
 ...
 * 
 * @param configuration The configuration of the device, this can be made by bitwise OR ('|') the enum @ref LP5024_Configuration
 * @param addressType the I2C address type to write to 
 */
void LP5024::Configure(uint8_t configuration, EAddressType addressType) {
    i2c1_write_byte(getAddress(addressType), DEVICE_CONFIG1, configuration & 0x3F);
}
 ...
 */
void LP5024::SetScaling(uint8_t scaling) {
    uint8_t buff;
    i2c1_read_byte(_i2c_address, DEVICE_CONFIG1, &buff);
    ...
    i2c1_write_byte(_i2c_address, DEVICE_CONFIG1, buff);
}
 ...
 */
void LP5024::SetPowerSaving(uint8_t powerSave) {
    uint8_t buff;
    i2c1_read_byte(_i2c_address, DEVICE_CONFIG1, &buff);
 ...
 */
void LP5024::SetAutoIncrement(uint8_t autoInc) {
    uint8_t buff;
    i2c1_read_byte(_i2c_address, DEVICE_CONFIG1, &buff);
 ...
 */
void LP5024::SetPWMDithering(uint8_t dithering) {
    uint8_t buff;
    i2c1_read_byte(_i2c_address, DEVICE_CONFIG1, &buff);
 ...
 */
void LP5024::SetMaxCurrentOption(uint8_t option) {
    uint8_t buff;
    i2c1_read_byte(_i2c_address, DEVICE_CONFIG1, &buff);
 ...
 */
void LP5024::SetGlobalLedOff(uint8_t value) {
    uint8_t buff;
    i2c1_read_byte(_i2c_address, DEVICE_CONFIG1, &buff);
 ...
 * @param enablePin 
 */
void LP5024::SetEnablePin(uint8_t enablePin) {
 ...
 */
void LP5024::SetLEDConfiguration(LED_Configuration ledConfiguration) {
    _led_configuration = ledConfiguration;
}
 ...
 * @param address 
 */
void LP5024::SetI2CAddress(uint8_t address) {
 ...
 * 
 * @param leds The LEDs to include in BANK control. See @ref LP5024_LEDS
 * @param addressType the I2C address type to write to
 * 
 * @note Code example could be `SetBankControl(LED_0 | LED_1 | LED_2 | LED_3);`
 */
void LP5024::SetBankControl(uint8_t leds, EAddressType addressType) {
    i2c1_write_byte(getAddress(addressType), LED_CONFIG0, leds);
}
 ...
 * @param addressType the I2C address type to write to
 */
void LP5024::SetBankBrightness(uint8_t brightness, EAddressType addressType) {
    i2c1_write_byte(getAddress(addressType), BANK_BRIGHTNESS, brightness);
}
 ...
 * @param addressType the I2C address type to write to
 */
void LP5024::SetBankColorA(uint8_t value, EAddressType addressType) {
    i2c1_write_byte(getAddress(addressType), BANK_A_COLOR, value);
}
 ...
 * @param addressType the I2C address type to write to
 */
void LP5024::SetBankColorB(uint8_t value, EAddressType addressType) {
    i2c1_write_byte(getAddress(addressType), BANK_B_COLOR, value);
}
 ...
 * @param addressType the I2C address type to write to
 */
void LP5024::SetBankColorC(uint8_t value, EAddressType addressType) {
    i2c1_write_byte(getAddress(addressType), BANK_C_COLOR, value);
}
 ...
 * @param addressType the I2C address type to write to
 */
void LP5024::SetBankColor(uint8_t red, uint8_t green, uint8_t blue, EAddressType addressType) {
    ...
    i2c1_write_multi(getAddress(addressType), BANK_A_COLOR, buff, 3);
}
 ...
 * @param addressType the I2C address type to write to
 */
void LP5024::SetLEDBrightness(uint8_t led, uint8_t brighness, EAddressType addressType) {
    i2c1_write_byte(getAddress(addressType), LED0_BRIGHTNESS + led, brighness);
}
 ...
 * @param addressType the I2C address type to write to
 */
void LP5024::SetOutputColor(uint8_t output, uint8_t value, EAddressType addressType) {
    i2c1_write_byte(getAddress(addressType), OUT0_COLOR + output, value);
}
 ...
 * @param addressType the I2C address type to write to
 */
void LP5024::SetLEDColor(uint8_t led, uint8_t red, uint8_t green, uint8_t blue, EAddressType addressType) {
    SetAutoIncrement(AUTO_INC_ON);
    ...
    i2c1_write_multi(getAddress(addressType), OUT0_COLOR + (led * 3), buff, 3);
}
 ...
 * @param addressType the I2C address type to write to
 */
void LP5024::WriteRegister(uint8_t reg, uint8_t value, EAddressType addressType) {
    i2c1_write_byte(getAddress(addressType), reg, value);
}
 ...
 */
void LP5024::ReadRegister(uint8_t reg, uint8_t *value) {
    i2c1_read_byte(_i2c_address, reg, value);
}
 ...
 */
uint8_t LP5024::getAddress(EAddressType addressType) {
    uint8_t i2c_address;
    switch (addressType)
    {
