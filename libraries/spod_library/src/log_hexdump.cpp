...
void log_hexdump(const char *tag, const void *buffer, uint16_t buff_len, esp_log_level_t log_level)
{
    if (buff_len > 0 && log_level <= ESP_LOG_VERBOSE) {
        ...
        do {
            ...
            switch (log_level) {
            case ESP_LOG_NONE:    LOGC(tag, "%s\r\n", hd_buffer); break;
            case ESP_LOG_ERROR:   LOGE(tag, "%s\r\n", hd_buffer); break;
            case ESP_LOG_WARN:    LOGW(tag, "%s\r\n", hd_buffer); break;
            case ESP_LOG_INFO:    LOGI(tag, "%s\r\n", hd_buffer); break;
            case ESP_LOG_VERBOSE:
            case ESP_LOG_DEBUG:   LOGD(tag, "%s\r\n", hd_buffer); break;
            default:              break;
            }
