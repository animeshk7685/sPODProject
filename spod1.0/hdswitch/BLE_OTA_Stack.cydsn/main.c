...
#define TIMEOUT_LENGTH  120

uint32 timeout = 0;

//CY_ISR_PROTO(TIMEOUT_ISR);
//
//CY_ISR(TIMEOUT_ISR)
//{
...
//    {
//        DBG_PRINT_TEXT("Bootloader timout. Application good. Launching.\r\n");
...
//        DBG_PRINT_DEC_TEXT(timeout, " \r\n");

        timeout++;      // each cycle ~0.5s
        
        if ((timeout > TIMEOUT_LENGTH) && (CYRET_SUCCESS == Loader_ValidateApp(1u)))
        {
            DBG_PRINT_TEXT("Bootloader timout. Application good. Launching.\r\n");
            Bootloadable_SetActiveApplication(1u);
            CySoftwareReset();
...
static void PrintProjectHeader()
{
    DBG_PRINT_TEXT("\r\n");
    DBG_PRINT_TEXT("\r\n");
    DBG_PRINT_TEXT("> BLE Upgradable Stack\r\n");
