...
void AfterImageUpdate(void)
{
    ...
    {
        myPrintf("Application project was updated. ");
        
        #if ((CYBLE_GAP_ROLE_PERIPHERAL || CYBLE_GAP_ROLE_CENTRAL) && (CYBLE_BONDING_REQUIREMENT == CYBLE_BONDING_YES))
            if (CYBLE_GATT_DB_CCCD_COUNT == (uint32) CY_GET_XTND_REG8((volatile uint8 *)STACK_UPDATE_FLAG_OFFSET))
            {
                myPrintf("CCCD number has not changed.\r\n");
            }
            else
            {
                myPrintf("CCCD number has changed.\r\n");
                myPrintf("Erasing bounding data...");
    ...
    #if ((CYBLE_GAP_ROLE_PERIPHERAL || CYBLE_GAP_ROLE_CENTRAL) && (CYBLE_BONDING_REQUIREMENT == CYBLE_BONDING_YES))
        #if (0 && PRINT_BOUNDING_DATA == YES)
            /* Print bounding data if it was enabled in options.h */
            if (CYBLE_GATT_DB_CCCD_COUNT > 0u)
            {
                myPrintf("CCCD array(s):\r\n");
                for (i = 0u; i <= CYBLE_GAP_MAX_BONDED_DEVICE; i++)
                {
                    myPrintf("%lu: ", i);
                    for (j = 0u; j < CYBLE_GATT_DB_CCCD_COUNT; j++)
                    {
//                        myPrintf("0x%02x ", cyBle_flashStorage.attValuesCCCDFlashMemory[i][j]);
                        myPrintf("0x%x ", cyBle_flashStorage.attValuesCCCDFlashMemory[i][j]);
                    }
                    myPrintf("\r\n");
                }
                myPrintf("\n%d, %d \r\n", (unsigned int) (cyBle_flashStorage.attValuesCCCDFlashMemory)/CY_FLASH_SIZEOF_ROW, i);
            }
            ...
            for (i = 0u; i < sizeof(cyBle_flashStorage.stackFlashptr); i++)
            {
                ...
                {
//                    myPrintf("0x%02x ", cyBle_flashStorage.stackFlashptr[i]);
                    myPrintf("0x%x ", cyBle_flashStorage.stackFlashptr[i]);
                    j++;
                }
                else
                {
//                    myPrintf("0x%02x\r\n", cyBle_flashStorage.stackFlashptr[i]);
            ...
            myPrintf("\n%d, %d \r\n", (unsigned int) (cyBle_flashStorage.stackFlashptr)/CY_FLASH_SIZEOF_ROW, i);
            myPrintf("\r\n");
