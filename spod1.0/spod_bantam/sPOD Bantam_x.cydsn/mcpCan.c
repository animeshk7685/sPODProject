...
void mcpCanReset()
{
    ...
    SPI_ss0_m_Write(0);
    SPI_SpiUartWriteTxData(MCP_RESET);
...
uint8 mcpCanReset2()
{
    ...
    while(SPI_SpiUartGetRxBufferSize() < 1){
        if((startRst - millis()) > 10)
        {
            UART_PutString("CAN RESET FAIL \r\n");
            SPI_ss0_m_Write(1);
...
void mcpCanSleep(uint8_t isSleep)
{
    uint8 res;
...
//        res = mcpCanSetCanCtrlMode(MODE_SLEEP);
        ...
        res = mcpCanInitSleep(CAN_250KBPS);
        
        //mcpCanSetRegister(MCP_CANINTE, MCP_WAKIF);
        
        if(res != MCP2515_OK)
        {
//            myPrintf("CAN Sleep Error...\n");
...
uint8 mcpCanReadRegister(uint8 address)
{
    uint8 buffer[] = {MCP_READ, address, 0xFF};
    ...
    buffer[0] = SPI_SpiUartReadRxData();
    buffer[1] = SPI_SpiUartReadRxData();
    buffer[2] = SPI_SpiUartReadRxData();
    
    return buffer[2];
}

void mcpCanReadRegisters(const uint8 address, uint8 *outBuffer, const uint8 n)
{
	uint8 i;
    uint8 packetLength = n + 2;
    uint8 buffer[packetLength];
    buffer[0] = MCP_READ;
    buffer[1] = address;
    
	SPI_SpiUartClearRxBuffer();
    SPI_ss0_m_Write(0);
    SPI_SpiUartPutArray(buffer, packetLength);
    
    while(SPI_SpiUartGetRxBufferSize() < packetLength){};
    SPI_ss0_m_Write(1);
	...
	for (i=0; i < n; i++) 
    {
		outBuffer[i] = SPI_SpiUartReadRxData();
	}
	
}

void mcpCanSetRegister(uint8 address, uint8 value)
{
    uint8 buffer[] = {MCP_WRITE, address, value};
    SPI_SpiUartClearRxBuffer();
    SPI_ss0_m_Write(0);
    SPI_SpiUartPutArray(buffer, 3);     
    while(SPI_SpiUartGetRxBufferSize() < 3){};
...
void mcpCanSetRegisters(uint8 address, uint8 *values, uint8 n)
{
    uint8 i;
    uint8 packetLength = n + 2;
    uint8 buffer[packetLength];
...
void mcpCanModifyRegister(uint8 address, uint8 mask, uint8 data)
{
    uint8 buffer[] = {MCP_BITMOD, address, mask, data};
    SPI_SpiUartClearRxBuffer();
    SPI_ss0_m_Write(0);
    SPI_SpiUartPutArray(buffer, 4); 
    while(SPI_SpiUartGetRxBufferSize() < 4){};
    SPI_ss0_m_Write(1);
}


uint8 mcpCanReadStatus(void)                             
{
	uint8 buffer[] = {MCP_READ_STATUS, 0xFF};
...
uint8 mcpCanConfigRate(const uint8 canSpeed)            
{
...
void mcpCanWriteId( const uint8 mcp_addr, const uint8 ext, const uint32 id )
{
    uint16_t canid;
    uint8 tbufdata[4];
...
void mcpCanReadId( const uint8 mcp_addr, uint8* ext, uint32* id )
{
    uint8 tbufdata[4];
...
void mcpCanWriteCanMsg( mcpCan *can, uint8 buffer_sidh_addr)
{
    uint8 mcp_addr;
...
void mcpCanReadCanMsg( mcpCan *can, uint8 buffer_sidh_addr)        /* read can msg                 */
{
    uint8 mcp_addr, ctrl;
...
void mcpCanStartTransmit(uint8 mcp_addr)              /* start transmit               */
{
    mcpCanModifyRegister( mcp_addr-1 , MCP_TXB_TXREQ_M, MCP_TXB_TXREQ_M );
}

uint8 mcpCanGetNextFreeTxBuf(uint8 *txbuf_n)                 /* get Next free txbuf          */
{
    uint8 res, i, ctrlval;
...
uint8 mcpCanInit(uint8 canSpeed)                       /* mcp2515init                  */
{
...
#if DEBUG_MODE
    UART_PutString("Reset Device success \r\n");
#endif    
...
uint8 mcpCanInitSleep(uint8 canSpeed)               /* setup sleep mode with wake on detect  */
{
    uint8 res;

    //myPrintf("mcpCanInitSleep\n");
    ...
    //myPrintf("(%d)MODE: %x\n", res, mcpCanReadRegister(MCP_CANSTAT));
                                                                        /* set baudrate                 */
    if(mcpCanConfigRate(canSpeed))
    {
      return res;
    ...
    if ( res == MCP2515_OK ) {
        ...
        mcpCanInitCANBuffers();
                                                                        /* interrupt mode               */
        //mcpCanSetRegister(MCP_CANINTE, MCP_RX0IF | MCP_RX1IF | MCP_WAKIF);
        mcpCanSetRegister(MCP_CANINTE, MCP_WAKIF);
        
        //myPrintf("MCP_CANINTE: %x\n", mcpCanReadRegister(MCP_CANINTE));
        
        mcpCanModifyRegister(MCP_CNF3, 0x80, 0x00); // clear unused SOF/CLKOUT bit to be able to set into sleep...
...
uint8 mcpCanInitMask(uint8 num, uint8 ext, uint32 ulData)
{
...
uint8 mcpCanInitFilt(uint8 num, uint8 ext, uint32 ulData)
{
    uint8 res = MCP2515_OK;
#if DEBUG_MODE
...
uint8 mcpCanSetMsg(mcpCan * can, uint32 id, uint8 ext, uint8 len, uint8 *pData)
{
    int i = 0;
    can->m_nExtFlg = ext;
    can->m_nID     = id;
...
uint8 mcpCanClearMsg(mcpCan * can)
{
    int i;
...
uint8 mcpCanSendMsg(mcpCan *can)
{
...
uint8 mcpCanSendMsgBuf(mcpCan *can, uint32 id, uint8 ext, uint8 len, uint8 *buf)
{
    mcpCanSetMsg(can, id, ext, len, buf);
    return mcpCanSendMsg(can);
}

uint8 mcpCanReadMsg(mcpCan *can)
{
    uint8 stat, res;
...
uint8 mcpCanReadMsgBuf(mcpCan * can, uint8 *len, uint8 *buf)
{
    int i;
    uint8 status = mcpCanReadMsg(can);
    *len = can->m_nDlc;
    for(i = 0; i < can->m_nDlc; i++)
    {
      buf[i] = can->m_nDta[i];
...
uint8 mcpCanCheckError()
{
    uint8 eflg = mcpCanReadRegister(MCP_EFLG);
...
uint32 mcpCanGetCanId(mcpCan * can)
{
    return can->m_nID;
}