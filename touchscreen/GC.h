...
#define GCDEBUG0 255 //switchoff debug
#define GCDEBUG1 1 //most critical debug information
#define GCDEBUG2 2 //mid critical debug information
#define GCDEBUG3 3 //least critical debug information

/* Change the below statement wrt debug criticality */
#define GCDEBUG GCDEBUG0

/* Version number of GC */
#define GC_MAJOR				1
#define GC_MINOR				2
#define GC_BUILD				0
...
/* GC status enum - used for api return type, error type etc */
...
extern uint8_t  GInit;
extern uint8_t PDNPin;
extern uint8_t TrnsFlag;//Global flag to indicate that initialization is done
extern int32_t  GError;//Global error flag
extern uint16_t IntPin;//interrupt pin	
extern uint8_t  DispGpioPin;
extern uint8_t AudioGpioPin;//default GC pin assignments for diaplay and audio control	
...
GC_DisplayResolution			Width 	Height	Swizzle	Polarity	PClk	HCycle	Hoffset		Hsync0		Hsync1		VCycle	Voffset		Vsync0	Vsync1	
...
typedef struct FT_Fonts
{
	...
	/* Bitmap format of font wrt bitmap formats supported by FT81x - L1, L4, L8 */
	uint32_t	FontBitmapFormat;
	/* Font line stride in FT81x ROM */
 ...
 /*
 Following are the assumptions made for GC library
 ...
 */
	...
	FT_Status GC_InitA(uint8_t ResType);//api to set the resolution of output display	
    FT_Status GC_Init_Soft(uint8_t ResType);     // same as InitA, but without the hardware reset
	...
	FT_Status GC_Exit(void);//exit state of graphics processor
	void GC_GetVersion(  uint8_t *Major, uint8_t *Minor, uint8_t *Build);/* Api to get version of the library */
    
	void GC_SetDisplayEnablePin(  uint8_t GpioBit);//GC gpio bit for display enable/disable
	void GC_SetAudioEnablePin(  uint8_t GpioBit);//GC gpio bit for audio enable/disable
	void GC_DisplayOn( );/* Apis to enable/disable backlight */
	void GC_DisplayOff( );
	void GC_AudioOn( );
	void GC_AudioOff( );
    void GC_SetGpioDrive(uint8_t port, uint8_t level);
	void GC_SetInterruptPin(  uint16_t Intpin);//apis to set interrupt pin
	void GC_ResetCopro( );//api to reset only coprocessor	
	void GC_Reset( );//api to reset whole GC via pdn - if pdn is not assigned then reset is not successful
    void GC_Reset_Soft();
	/* Apis related to power up/power down funcationality */
	void GC_DisplayConfigExternalClock(  uint8_t ResType);
	void GC_ActiveInternalClock( );
	void GC_PDN_Cycle( );
	
	/* Apis related to graphics processor */	
	//enable or disable interrupts
	void GC_EnableInterrupts(  uint8_t GEnable,uint8_t Mask);
	uint8_t GC_ReadIntReg( );//read the interrupt flag register - note that on GC the interrupts are clear by read
	...
	FT_GEStatus GC_RestoreContext( );
	FT_GEStatus GC_Return( );
	FT_GEStatus GC_SaveContext( );
	...
	FT_GEStatus GC_Cmd_LoadIdentity(  );
	...
	FT_GEStatus GC_Cmd_ScreenSaver( );
	...
	FT_GEStatus GC_Cmd_Stop( );
	FT_GEStatus GC_Cmd_Swap( );
	...
	FT_AEStatus GC_PlaySound(  uint16_t SoundNote);//higher byte is the note and lower byte is the sound
	void GC_StopSound( );//volume will not be modified
	void GC_SetSoundVolume(  uint8_t Volume);
	uint8_t GC_GetSoundVolume( );

	FT_AEStatus GC_PlayAudio(  uint8_t Volume,uint8_t Format,uint16_t SamplingFreq,uint32_t BufferAddr,uint32_t BufferSize,uint8_t Loop);//one shot or continuous, sampling frequency is from 8k to 48k
	void GC_SetAudioVolume(  uint8_t Volume);
	FT_AEStatus GC_GetAudioStats(  uint32_t *CurrPlayAddr);//returns playback stopped or continue
	uint8_t GC_GetAudioVolume( );
	void GC_StopAudio( );//volume will not be modified

	/* Apis related to touch engine */
	void GC_SetTouchMode(  uint8_t TMode);//one of 0ff/oneshot/frame/continuous. default being continuous
	...
	void GC_SetHostTagXY(  uint16_t xoffset,uint16_t yoffset);//api to set coordinates for host specific tag query
	uint8_t GC_GetHostTagXY( );//api to get TAG from GC for coordinates set by  SetHostTagXY() api - host needs to wait for at least 1 frame to get these query values
	void GC_GetTagXY(  sTagXY *sTagxy);//get the touched object tag and repective xy coordinates
	void GC_GetTrackTag(  sTrackTag *sTracktag);//get the track value and the tag value
	...
	void GC_DLStart( );//inserts cmd_dlstart() followed by clear(1,1,1) graphics command
	void GC_DLEnd( );//inserts display() gpu instruction at the end and inserts cmd_swap() command
	FT_GEStatus GC_CheckLogo( );//special api to check logo completion
	...
	FT_GEStatus GC_WriteCmd_Src(  uint8_t *Src,uint32_t NBytes);//api to send N bytes to command 	
	...
	FT_GEStatus GC_Cmd_GetResult(  uint32_t *Result);//reads the result of the previous commands such as cmd_memcrc,cmd_calibration, cmd_regread which has return values. if busy returns busy status
	FT_GEStatus GC_Cmd_GetResults(  int8_t *pA,uint16_t NBytes);//reads N bytes of result bytes from current write pointer
    FT_GEStatus GC_WriteCmdfromflash( const uint8_t *Src,uint32_t NBytes);
